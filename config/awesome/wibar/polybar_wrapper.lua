...
return function(opts)
  ...
  local buttons = gears.table.join(
    awful.button({ "Shift" }, keys.left_click, function()
      widget.update()
    end)
  )
  ...
  if opts.left_click then
    buttons = gears.table.join(
      buttons,
      awful.button({}, keys.left_click, compose_update(opts.left_click))
    )
  end
  ...
  if opts.right_click then
    buttons = gears.table.join(
      buttons,
      awful.button({}, keys.right_click, compose_update(opts.right_click))
    )
  end
  ...
  if opts.middle_click then
    buttons = gears.table.join(
      buttons,
      awful.button({}, keys.middle_click, compose_update(opts.middle_click))
    )
  end
  ...
  local function widget_loop()
    awful.spawn.with_line_callback(opts.command, {
